...
class App {
                ...
                allForms[i].addEventListener('submit', async function (e) {
                    ...
                    for (let i = 0; i < elements.length; i++) {
                        ...
                        if (inputElement.type !== 'submit') {
                            ...
                            const classOfElement = typeof inputElement.classList.value == 'string' &&
                                inputElement.classList.value.length > 0
                                ? inputElement.classList.value
                                : '';
                            const valueOfElement = inputElement.type == 'checkbox' &&
                    ...
                    // Call the API
                    try {
                        const responsePayload = await self.client.request(undefined, path, method, queryStringObject, payload);
                        ...
                        const formErrorElement = document.querySelector('#' + formId + ' .formError');
     ...
     */
    async formResponseProcessor(formId, requestPayload, responsePayload) {
        ...
        if (formId == 'accountCreate') {
            ...
            try {
                const newResponsePayload = await this.client.request(undefined, 'api/tokens', 'POST', undefined, newPayload);
     ...
     */
    async renewToken() {
        const currentToken = typeof this.config.sessionToken == 'object' ? this.config.sessionToken : false;
        try {
            if (currentToken) {
                ...
                const responsePayload = await this.client.request(undefined, 'api/tokens', 'GET', queryStringObject, undefined);
                this.setSessionToken(responsePayload);
            }
            else {
    ...
    async loadAccountEditPage() {
        ...
        const phone = this.config.sessionToken && typeof this.config.sessionToken.phone == 'string'
        ...
        if (phone) {
            ...
            try {
                const responsePayload = await this.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined);
        ...
        if (phone) {
            ...
            try {
                ...
                for await (const checkId of allChecks) {
                    ...
                    try {
                        const responsePayload = await this.client.request(undefined, 'api/checks', 'GET', newQueryStringObject, undefined);
                        ...
                        const state = typeof responsePayload.state == 'string'
    ...
    async loadChecksEditPage() {
        ...
        if (id) {
            ...
            try {
                const responsePayload = await this.client.request(undefined, 'api/checks', 'GET', queryStringObject, undefined);
    ...
    async logUserOut(redirectUser) {
        ...
        const tokenId = this.config.sessionToken && typeof this.config.sessionToken.id == 'string'
        ...
        try {
            await this.client.request(undefined, 'api/tokens', 'DELETE', queryStringObject, undefined);
...
class Client {
     ...
     */
    async request(headers, path, method, queryStringObject, payload) {
        // Set defaults
