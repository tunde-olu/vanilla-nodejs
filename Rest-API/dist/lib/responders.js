...
class Responders {
    ...
    stats() {
        ...
        const stats = {
            ...
            'Allocated Heap Used (%)': Math.round((v8.getHeapStatistics().used_heap_size / v8.getHeapStatistics().total_heap_size) *
                100),
            'Available Heap Allocated (%)': Math.round((v8.getHeapStatistics().total_heap_size / v8.getHeapStatistics().heap_size_limit) *
                100),
    ...
    async listUsers(flag) {
        try {
            ...
            for (const userId of userIds) {
                ...
                if (!userData)
                    throw 'Error fetch the users';
                const numOfChecks = typeof userData.checks === 'object' && userData.checks instanceof Array
    ...
    async moreUserInfo(str) {
        ...
        try {
            ...
            if (!userId) {
                console.log('\x1b[33m%s\x1b[0m', 'You need to provide a flag with --{userId} to look up a user');
                return;
                // throw 'You need to provide a flag with --{userId} to look up a user';
            }
            const userData = await _data.read('users', userId, () => { });
            if (!userData)
                throw `No user exist with the id: ${userId}`;
    ...
    async listChecks(str) {
        try {
            ...
            for (const checkId of checkIds) {
                ...
                const checkOrUnknown = typeof checkData.state === 'string' ? checkData.state : 'unknown';
                ...
                if (!flag || flag.includes(state)) {
                    let line = `ID: ${checkData.id} ${checkData.method?.toUpperCase()} ${checkData.protocol}://${checkData.url} State: ${checkOrUnknown}`;
                    console.log(line);
                    formatConsole.verticalSpace();
                }
            }
     ...
     * More check info
     */
    async moreCheckInfo(str) {
        // Get the checkId from the str string
        try {
            const checkId = str.split('--')[1]?.trim();
            if (!checkId) {
                console.log('\x1b[33m%s\x1b[0m', 'You need to provide a flag with --{checkId} to look up a user');
                return;
                // throw 'You need to provide a flag with --{checkId} to look up a user';
            }
            const checkData = await _data.read('checks', checkId, () => { });
            if (!checkData)
                throw `No check exist with the id: ${checkId}`;
     ...
     * More log info
     */
    async moreLogInfo(str, testing) {
        // Get the checkId from the str string
        try {
            const logFileName = str.split('--')[1]?.trim();
            if (!logFileName) {
                console.log('\x1b[36m%s\x1b[0m', 'You need to provide a flag with --{filename} of the log you want to read');
                return;
