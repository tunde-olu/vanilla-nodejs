...
class Helpers {
    ...
    async verifyToken(id, phone, callback) {
        // Lookup the token
     ...
     */
    sendTwilioSms(_phone, _msg, callback) {
        // Validate parameters
        ...
        const msg = typeof _msg === 'string' && _msg.trim().length > 0 && _msg.length <= 1600
            ? _msg
            : false;
    ...
    async getTemplate(templateName, data) {
        ...
        try {
            ...
            const htmlString = await fsPromises.readFile(`${templatesDir}${templateName}.html`, 'utf-8');
            ...
            throw new FileSystemError(error.message || err);
        }
    }
    /**
     ...
     */
    async addUniversalTemplates(str, data) {
        ...
        try {
            ...
            const err = error;
            throw new FileSystemError(err.message || error);
        }
    }
    /**
