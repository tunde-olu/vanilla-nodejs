import type { IHandlers, RequestMethods } from '../types/index.js';
...
handlers.users = function (data, callback) {
	...
	if (acceptableMethods.indexOf(data.method!) > -1) {
		handlers._users![data.method as RequestMethods](data, callback);
	} else {
		callback(405);
	}
};
...
handlers._users!.get = function (data, callback) {
	...
	if (phone) {
		...
		if (token) {
			handlers._verifyToken!(token, phone, (tokenIsValid) => {
				if (tokenIsValid) {
					// Lookup the user
					_data.read!('users', phone, (err, data) => {
						if (!err && data) {
...
handlers._verifyToken = function (id: string, phone: string, callback) {
	// Lookup the token
	_data.read!('tokens', id, (err, data) => {
